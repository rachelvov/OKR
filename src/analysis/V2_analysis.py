...
number_of_tweets=sum([len(graph.sentences)for graph in all_graphs])
print ("number of tweets: "+str(number_of_tweets))

number_of_tokens=sum([sum([len(s) for s in graph.sentences.values()])for graph in all_graphs])
...
average_number_of_edge_terms=np.average([l for l in [len(set([m.terms for m in edge.mentions.values() if not m.terms==""])) for graph in all_graphs for edge in graph.edges.values() if len(edge.mentions)>1 ] if not l==0])
print ("average_number_of_edge_terms: "+ str(average_number_of_edge_terms))

nodes_of_edges=[graph.name+edge.nodes_pair[0] for graph in all_graphs for edge in graph.edges.values()]
nodes_of_edges+=[graph.name+edge.nodes_pair[1] for graph in all_graphs for edge in graph.edges.values()]
